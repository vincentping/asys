...
29. [ASys 与驱动模型的同构性：聪明的驱动程序](#29-asys-与驱动模型的同构性聪明的驱动程序)
...
**外部视角补充**：JSON 当然也能审计，这不是选择二进制帧的理由。核心理由是**语义确定性**：JSON 里 `"pid": "1234"` 和 `"pid": 1234` 都合法，解析器行为可能不同；二进制帧里 PID 是固定偏移的 4 字节 `uint32`，无歧义，无注入面，任何实现解出来都一样。一句话：**JSON 适合人读，APDU 适合机器处理。Agent 不需要"人能读"这个特性。**
...
## 29. ASys 与驱动模型的同构性：聪明的驱动程序

驱动程序的本质是：把硬件操作抽象成结构化接口，让上层软件无需关心底层细节。内核通过 IOCTL 向用户态暴露设备能力，固定的命令码、结构化的参数、明确的操作对象——这正是 APDU 的设计哲学。

ASys 做的是同一件事，只是服务的对象从内核换成了 AI Agent：把系统操作抽象成 Agent 能直接理解的结构化指令，让 Agent 无需关心底层是什么发行版、什么内核版本。

**与 IOCTL 的同构性：**

| IOCTL（设备驱动） | ASys |
|-----------------|------|
| 命令码（`SIOCGIFADDR` 等） | INS 字节（`0x02 SYS_STATUS` 等） |
| 参数结构体 | P1/P2 + Data 字段 |
| 操作对象（设备文件） | 节点（由连接标识） |
| 返回值 / errno | SW 状态字（`0x9000` / `0x69xx`） |

**与 ADR-28 USB 类比的关系：**

ADR-28 描述的是 ASys 指令集的**分层结构**（Core ISA 如同 USB 核心协议，扩展如同设备描述符）。本节描述的是 ASys **整体定位**的同构性——ASys 之于 Agent，如同驱动程序之于内核：向上暴露统一接口，向下屏蔽硬件差异。

两个类比互补，不冲突：USB 类比说的是"怎么组织指令集"，驱动类比说的是"这个东西在系统栈里是什么角色"。

**实践含义：**

这个类比也指出了 ASys 的自然延伸方向——AI 读芯片手册生成 HAL，再套一层 ASys 风格的统一指令集，Agent 即可跨芯片操作嵌入式设备，无需关心底层是 STM32 还是 ESP32。驱动层的"屏蔽硬件差异"能力，在 Agent 时代有了新的应用场景。

---

## 30. Why Per-Instruction Auth Tag Inside the Noise Channel?

> 记录于 2026-06-10。来源：对"指令级 Auth Tag 是否为密码学冗余"这一质疑的回应。

**决策**：保留 Standard ISA 及更高安全级别指令的指令级 Auth Tag（`HMAC-BLAKE2b`，见 `asys-spec.md` 第2.2.3节），定位为纵深防御与帧格式稳定性承诺，而非传输层完整性的必要补充。

**背景与质疑**：

握手完成后，所有 APDU 帧都在 Noise IK 建立的 ChaCha20-Poly1305 AEAD 通道内传输（见 `asys-spec.md` 第2.1节）。AEAD 本身已经保证了每一帧的机密性与完整性——任何被篡改的密文在解密阶段即验证失败。Noise transport 的 nonce 逐帧递增，加之每次握手派生独立的会话密钥，使得截获的密文无论在会话内还是跨会话重放，都无法在新的 nonce/密钥下正确解密。

由此产生一个合理的质疑：既然 AEAD 通道已经提供了完整性与抗重放，那么在其之上再为每条指令计算一次 `HMAC-BLAKE2b`（见第2.2.3节）是否纯属冗余？

必须正面承认：**对一个处于 Noise 通道之外的网络攻击者而言，指令级 Auth Tag 带来的防御增量近似为零。** 网络攻击者既无法读取明文，也无法构造能通过 AEAD 解密的密文；指令真实性事实上已由 AEAD 层兜底。本 ADR 不试图论证这一层在密码学上"必要"，而是说明在防御增量近似为零的前提下，为何仍然保留它。

**保留理由（纵深防御与帧格式稳定性）**：

第一，防传输层实现的单点失效。`asyd` 的 Noise IK 状态机是在 Monocypher 之上自行实现的（见第7节、第15节），不是经过广泛审计的成熟库。第15节记录的三个跨语言互操作 bug 中，两处 DH 方向错误在单语言测试中相互抵消，只有跨实现互操作才暴露——这正说明手写传输层存在静默缺陷的现实可能。一旦 Noise/Monocypher 实现层出现缺陷（例如 nonce 复用或状态机错误），AEAD 提供的完整性保证就可能局部失效。指令级 Auth Tag 用一份独立计算的 MAC，使得传输层的单点失效不会直接导致指令完整性失守。这是纵深防御的标准动机：不把指令真实性 100% 押在单一加密层的实现正确性上。

第二，帧格式稳定性承诺。CLA 字节的安全位（`Sec`，bit3-2）与帧尾 16 字节 Auth Tag 槽位，是 ASys 自 ISO 7816 APDU 继承而来的帧结构组成部分（见第5节、`asys-spec.md` 第3.1.2节）。协议演进准则要求字段偏移在大版本内不变（见 `asys-spec.md` 第1.9节）。此刻删除 Auth Tag 槽位与 `Sec` 语义、未来若需恢复，将构成两次 breaking change——与"二进制是永恒的"这一核心承诺直接冲突。保留一个当前防御增量有限但语义清晰的槽位，成本远低于反复改动帧格式。

第三，成本可忽略。`HMAC-BLAKE2b` 与 Noise IK 的 KDF 共用同一底层构造（见第14节），单次 MAC 计算在 `asyd` 的负载量级下成本可忽略，不构成核心请求路径的性能瓶颈。

**成本核算（诚实记录）**：

保留这一层确有代价，如实记录：

- 每条 Standard ISA 指令在执行前多一次 `HMAC-BLAKE2b` 计算与一次常量时间比较；
- 安全帧 Data 段被 `Seq` 占用 4 字节（见第2.2.2节）；
- SDK 实现复杂度上升——SDK 须正确派生 `Epoch_ID`、维护 `Seq`、按第2.2.3节的字段拼接顺序计算 MAC。

需要明确区分的是：**`Seq` 是应用层的去重与幂等机制，不属于本 ADR 讨论的"密码学冗余"。** `Seq` 支撑第2.2.4节的会话内防重放与 SDK 重传场景下的 exactly-once 语义（见第23节对 `0x6985` 的处理），其价值独立于本 ADR——即便指令级 MAC 被论证为冗余，`Seq` 也不应因此移除。

**Epoch_ID 的诚实定位**：

`Epoch_ID` 在第2.2.3节的设计目标是使跨会话重放在密码学上失效。但在 Noise AEAD 通道内，跨会话重放已经在解密层失效——旧会话密文在新会话密钥下无法解密。因此 `Epoch_ID` 混入 Auth Tag 计算，在 AEAD 通道内**没有独立的安全价值**，其防御目标已被传输层覆盖。保留 `Epoch_ID` 仅为签名算法定义的完整性与一致性——它是第2.2.3节签名输入的固定组成部分，移除它会改变 MAC 输入布局而无任何安全收益。这一点不回避：`Epoch_ID` 的实际作用与指令级 Auth Tag 同源——在 AEAD 通道内是纵深防御的组成部分，而非独立必需的密码学机制。

**结论**：指令级 Auth Tag 对网络攻击者的边际防御接近于零，这一点如实承认。保留它的理由是纵深防御（不信任手写传输层的单点正确性）与帧格式稳定性（不为有限收益反复改动继承自 APDU 的帧结构），而非传输层完整性的必要补充。`Seq` 的应用层去重价值独立于此，不受本 ADR 影响。
